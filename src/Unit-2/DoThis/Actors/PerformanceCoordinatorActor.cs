...
namespace ChartApp.Actors
{
    ...
    public class PerformanceCounterCoordinatorActor : ReceiveActor
    {
        ...
        public class Rotate
        {
            public Rotate(CounterType oldCounter, CounterType newCounter)
            {
                OldCounter = oldCounter;
                NewCounter = newCounter;
            }

            public CounterType OldCounter { get; private set; }
            public CounterType NewCounter { get; private set; }
        }
            ...
            CounterGenerators = new Dictionary<CounterType, Func<PerformanceCounter>>()
        {
            ...
            {CounterType.Ram, () => new PerformanceCounter("Memory",
            ...
            new Dictionary<CounterType, Func<Series>>()
        {
            ...
            {CounterType.Ram, () =>
            new Series(CounterType.Ram.ToString()){
            ...
            Dictionary<CounterType, IActorRef> counterActors)
        {
            ...
            Receive<Rotate>(rotate =>
            {
                if (_counterActors.ContainsKey(rotate.OldCounter))
                {
                    _counterActors[rotate.OldCounter].Tell(new UnsubscribeCounter(rotate.OldCounter, Sender));
                }
                if (_counterActors.ContainsKey(rotate.NewCounter))
                {
                    _counterActors[rotate.NewCounter].Tell(new SubscribeCounter(rotate.NewCounter, Sender));
                }
            });
        }
